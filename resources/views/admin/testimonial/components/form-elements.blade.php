    ...
    @foreach($locales as $locale)
        <div class="col-md" v-show="shouldShowLangGroup('{{ $locale }}')" v-cloak>
            <div class="form-group row align-items-center" :class="{'has-danger': errors.has('title_{{ $locale }}'), 'has-success': fields.title_{{ $locale }} && fields.title_{{ $locale }}.valid }">
                <label for="title_{{ $locale }}" class="col-md-2 col-form-label text-md-right">{{ trans('admin.testimonial.columns.title') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    <input type="text" v-model="form.title.{{ $locale }}" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('title_{{ $locale }}'), 'form-control-success': fields.title_{{ $locale }} && fields.title_{{ $locale }}.valid }" id="title_{{ $locale }}" name="title_{{ $locale }}" placeholder="{{ trans('admin.testimonial.columns.title') }}">
...
<div class="row">
    @foreach($locales as $locale)
        <div class="col-md" v-show="shouldShowLangGroup('{{ $locale }}')" v-cloak>
            <div class="form-group row align-items-center" :class="{'has-danger': errors.has('job_{{ $locale }}'), 'has-success': fields.job_{{ $locale }} && fields.job_{{ $locale }}.valid }">
                <label for="job_{{ $locale }}" class="col-md-2 col-form-label text-md-right">{{ trans('admin.testimonial.columns.job') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    <input type="text" v-model="form.job.{{ $locale }}" v-validate="''" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('job_{{ $locale }}'), 'form-control-success': fields.job_{{ $locale }} && fields.job_{{ $locale }}.valid }" id="job_{{ $locale }}" name="job_{{ $locale }}" placeholder="{{ trans('admin.testimonial.columns.job') }}">
                    <div v-if="errors.has('job_{{ $locale }}')" class="form-control-feedback form-text" v-cloak>{{'{{'}} errors.first('job_{{ $locale }}') }}</div>
                </div>
            </div>
        </div>
    @endforeach
    ...
    @foreach($locales as $locale)
        <div class="col-md" v-show="shouldShowLangGroup('{{ $locale }}')" v-cloak>
            <div class="form-group row align-items-center" :class="{'has-danger': errors.has('text_{{ $locale }}'), 'has-success': fields.text_{{ $locale }} && fields.text_{{ $locale }}.valid }">
                <label for="text_{{ $locale }}" class="col-md-2 col-form-label text-md-right">{{ trans('admin.testimonial.columns.text') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    <div>
...
<div class="form-check row" :class="{'has-danger': errors.has('is_published'), 'has-success': fields.is_published && fields.is_published.valid }">
    <div class="ml-md-auto" :class="isFormLocalized ? 'col-md-8' : 'col-md-10'">
        ...
        <label class="form-check-label" for="is_published">
            {{ trans('admin.testimonial.columns.is_published') }}
        </label>
