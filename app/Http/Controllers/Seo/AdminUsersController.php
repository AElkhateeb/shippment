...
use App\Http\Requests\Seo\AdminUser\DestroyAdminUser;
use App\Http\Requests\Seo\AdminUser\ImpersonalLoginAdminUser;
use App\Http\Requests\Seo\AdminUser\IndexAdminUser;
use App\Http\Requests\Seo\AdminUser\StoreAdminUser;
use App\Http\Requests\Seo\AdminUser\UpdateAdminUser;
...
class AdminUsersController extends Controller
{
    ...
    public function index(IndexAdminUser $request)
    {
        ...
        return view('seo.admin-user.index', ['data' => $data, 'activation' => Config::get('seo-auth.activation_enabled')]);
    }
    ...
    public function create()
    {
        ...
        return view('seo.admin-user.create', [
            'activation' => Config::get('seo-auth.activation_enabled'),
            'roles' => Role::where('guard_name','seo')->get(),
        ]);
    }
    ...
    public function store(StoreAdminUser $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('seo/admin-users'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('seo/admin-users');
    }
     ...
     * @return void
     */
    public function show(SeoAdmin $adminUser)
    {
     ...
     * @return Factory|View
     */
    public function edit( SeoAdmin $adminUser)
    {
        $this->authorize('admin.admin-user.edit', $adminUser);

        $adminUser->load('roles');

        return view('seo.admin-user.edit', [
            'adminUser' => $adminUser,
            'activation' => Config::get('seo-auth.activation_enabled'),
            'roles' => Role::where('guard_name','seo')->get(),
        ]);
